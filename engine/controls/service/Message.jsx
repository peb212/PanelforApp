...
class MessageFactory
{
	 ...
	 * @param    {Object}                   data      [description]
	 * @param    {Number}                   action    [description]
	 * @return   {[type]}                             [description]
	 */
	sendToApp(actionkey="",data={},action=12)
	{
		var msg = {
			actionkey:actionkey,
			action:action,
		...
		if (window.webkit) { // ios 8.0+
			window.webkit.messageHandlers.observer.postMessage(msg);
	    } else if (window.Android) { // android
	    	Android.webMessageHandler(JSON.stringify(msg));
	    } else if (window.WebViewJavascriptBridge) { // ios 7.0, app 1.0.0 - app 1.1.0
	    	WebViewJavascriptBridge.callHandler('articleDetailH5GotoDispatch', msg);
	    }
	}
	/**
	 * [send description]
